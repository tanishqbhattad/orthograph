...
/* ---------------- the wall node map ----------------
   Every wall end, bucketed by position, so a junction can be solved by looking
   at one bucket instead of at every wall.

   It used to be thrown away whenever DOCV moved, which is on every mut(). That
   is free until something asks for wall geometry between two mutations — and
   moving a selection does exactly that — at which point the whole map is
   rebuilt from every wall in the drawing, once per wall. Two thousand walls
   measured 23.6 SECONDS; the mutations themselves took 1ms.

   So it is patched instead. mut() runs BEFORE the entity changes, which is the
   one moment the wall's OLD position is still known, so its two entries can be
   lifted out precisely then and put back at the new position on the next read.
   Anything structural — an add, a delete, a whole new document — still forces
   a full rebuild, because then the map cannot be reasoned about. */
let WNODES = null, WNODESv = -1, WNODE_OK = false;
const WNODE_PENDING = new Set();
function wallNodesRebuild() {
  const m = new Map();
  ...
  WNODES = m; WNODESv = DOCV; WNODE_OK = true; WNODE_PENDING.clear();
  return m;
}
/** Nothing derived from the walls can be reasoned about any more: rebuild the
    lot on the next read. */
function wallCacheInvalidate() {
  WNODE_OK = false; WNODE_PENDING.clear();
  WALLIST_OK = false;
  WIDX_OK = false; WIDX_PENDING.clear();
  OPENIDX_OK = false; OPENIDX_PENDING.clear();
}
/** an add or delete of one of these is what "structural" means */
function wallCacheStructural(e) {
  return !!e && (e.t === 'wall' || e.t === 'door' || e.t === 'window');
}
/** Called from mut() while the entity still holds its OLD position, which is
...
function wallNodeTouch(w) {
  if (!WNODES || !WNODE_OK) return;
  for (const end of [0, 1]) {
    const p = end ? w.b : w.a;
    if (!p) continue;
    const k = nk(p[0]) + ':' + nk(p[1]);
    const a = WNODES.get(k);
    if (!a) continue;
    const i = a.findIndex(x => x.w === w && x.end === end);
    if (i >= 0) a.splice(i, 1);
    if (!a.length) WNODES.delete(k);
  }
  WNODE_PENDING.add(w);
}
function wallNodes() {
  if (!WNODES || !WNODE_OK) return wallNodesRebuild();
  if (WNODE_PENDING.size) {
    for (const w of WNODE_PENDING) {
      /* a wall deleted since it was touched simply does not go back in */
      if (DOC.ents.get(w.id) !== w) continue;
      for (const end of [0, 1]) {
        const p = end ? w.b : w.a;
        if (!p) continue;
        const k = nk(p[0]) + ':' + nk(p[1]);
        let a = WNODES.get(k); if (!a) WNODES.set(k, a = []);
        a.push({ w, end, p });
      }
    }
    WNODE_PENDING.clear();
...
/* Which walls exist cannot change because one of them moved, so this is NOT
   keyed on DOCV: only an add, a delete or a new document disturbs it. */
let WALLIST = null, WALLIST_OK = false;
function allWalls() {
  if (WALLIST && WALLIST_OK) return WALLIST;
  WALLIST = [...DOC.ents.values()].filter(e => e.t === 'wall');
  WALLIST_OK = true; return WALLIST;
}
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
let WIDX = null, WIDX_OK = false;
const WIDX_PENDING = new Set();
...
/** run fn over every cell key the wall's raw box covers, or '*' if that is an
    absurd number of them */
function widxKeys(w, cell, fn) {
...
/** Lift a wall out of the cells its OLD position put it in. The cell size is
    left alone: insert and lookup only have to agree with each other, and a
    stale bucket size costs a little breadth, never a wrong answer. */
function wallIndexTouch(w) {
  if (!WIDX || !WIDX_OK) return;
  widxKeys(w, WIDX.cell, k => {
    const a = WIDX.cells.get(k);
    if (!a) return;
    const i = a.indexOf(w);
    if (i >= 0) a.splice(i, 1);
    if (!a.length) WIDX.cells.delete(k);
  });
  WIDX_PENDING.add(w);
}
function widxInsert(ix, w) {
  widxKeys(w, ix.cell, k => {
    let a = ix.cells.get(k); if (!a) ix.cells.set(k, a = []);
    a.push(w);
  });
}
function wallIndex() {
  if (WIDX && WIDX_OK) {
    if (WIDX_PENDING.size) {
      for (const w of WIDX_PENDING) {
        if (DOC.ents.get(w.id) !== w) continue;   /* deleted since: stays out */
        widxInsert(WIDX, w);
        /* the search radius may only ever grow between rebuilds */
        const t = wallT(w); if (t > WIDX.maxT) WIDX.maxT = t;
      }
      WIDX_PENDING.clear();
    }
    return WIDX;
  ...
  WIDX = { cell, cells: new Map(), walls, maxT };
  for (const w of walls) widxInsert(WIDX, w);
  WIDX_OK = true; WIDX_PENDING.clear();
  return WIDX;
}
...
let OPENIDX = null, OPENIDX_OK = false;
const OPENIDX_PENDING = new Set();
/** Called with the opening's OLD host still on it. Sliding a door along its
    wall does not change this map at all, but rehosting one does, and only the
    old value says which bucket to take it out of. */
function openIndexTouch(o) {
  if (!OPENIDX || !OPENIDX_OK) return;
  const a = OPENIDX.get(o.host);
  if (a) {
    const i = a.indexOf(o);
    if (i >= 0) a.splice(i, 1);
    if (!a.length) OPENIDX.delete(o.host);
  }
  OPENIDX_PENDING.add(o);
}
function openingsByHost() {
  if (OPENIDX && OPENIDX_OK) {
    if (OPENIDX_PENDING.size) {
      for (const o of OPENIDX_PENDING) {
        if (DOC.ents.get(o.id) !== o) continue;
        let a = OPENIDX.get(o.host); if (!a) OPENIDX.set(o.host, a = []);
        a.push(o);
      }
      OPENIDX_PENDING.clear();
    }
    return OPENIDX;
  }
  const m = new Map();
  ...
  OPENIDX = m; OPENIDX_OK = true; OPENIDX_PENDING.clear(); return m;
}
...
/* ============================================================
   The patch where three or more walls meet
   ------------------------------------------------------------
   Every wall's body stops at its own mitre. With two walls that closes the
   corner exactly, because they share both mitre points. With three or more it
   does not: the patch between their cut ends belongs to no wall, so the
   background shows through it — a white triangle at a T, a white star in the
   middle of a six-way junction. It was always there; a faint wash hid it and a
   solid poche does not.

   The patch is the ring walked in order, taking each end's two corners. Two
   walls that mitre share a corner, so those points collapse; two walls whose
   faces are parallel — a straight run passing through a junction — have no
   mitre between them, and their two square corners become an edge of the patch
   on their own. That is why this is built from the solved ends rather than
   from the mitre points: the mitre is missing in exactly the case the corners
   are needed.
   ============================================================ */
function nodePatch(P, lvl) {
  const ring = nodeRing(P, lvl);
  if (ring.length < 3) return null;                /* a corner closes itself */
  const pts = [];
  for (const r of ring) {
    const e = wallEndPoints(r.w, r.end);
    if (!e || !e.minus || !e.plus) return null;
    pts.push(e.minus, e.plus);
  }
  /* drop the shared mitres, and the seam where the walk comes back round */
  const ring2 = [];
  for (const p of pts) {
    const last = ring2[ring2.length - 1];
    if (!last || dist(last, p) > 1e-6) ring2.push(p.slice());
  }
  while (ring2.length > 1 && dist(ring2[0], ring2[ring2.length - 1]) <= 1e-6) ring2.pop();
  if (ring2.length < 3) return null;
  return Math.abs(polyArea(ring2)) > 1e-9 ? ring2 : null;
}
/** Which wall draws the patch. One of them has to, or it is drawn once per
    wall on the node — harmless while the fill is opaque and wrong the moment
    anything is drawn through it. The lowest id is a stable answer that every
    wall on the node agrees on without being told. */
function nodePatchOwner(P, lvl) {
  const ring = nodeRing(P, lvl);
  if (ring.length < 3) return null;
  let best = null;
  for (const r of ring) if (best == null || r.w.id < best) best = r.w.id;
  return best;
}
...
function wallShapes(w) {
  ...
  /* The fill comes first so a renderer can lay the poche down before anything
     is drawn over it — and it stops where the face lines stop. It did not: one
     polygon ran the whole length of the wall, straight on behind every reveal.
     A faint wash hid that. A solid poche does not, and a doorway came out as a
     block of wall with a swing arc drawn over the top of it.

     Both kinds of opening are cut out here. A window then gets its body drawn
     back ghosted a few lines down, which is what pocheGhost has always been
     for and only ever made sense against a poche that was cut to begin with. */
  const Lcar = { a: E0.plus, b: E1.minus }, Rcar = { a: E0.minus, b: E1.plus };
  /* The mitred corners are the wall's real ends; only an interior edge, made
     by a reveal, is square across the wall. Taking the end from the carrier
     would square off every corner and reopen the mitre. */
  const leftAt = (d) => d <= 1e-6 ? E0.plus : d >= L - 1e-6 ? E1.minus : faceAtD(Lcar, w, u, n, d);
  const rightAt = (d) => d <= 1e-6 ? E0.minus : d >= L - 1e-6 ? E1.plus : faceAtD(Rcar, w, u, n, d);
  ...
  for (const [rs, re] of cutRuns(L, [...doorCuts, ...winCuts])) {
    const l0 = leftAt(rs), l1 = leftAt(re), r1 = rightAt(re), r0 = rightAt(rs);
    if (pocheStack) {
      const tot = pocheStack.reduce((sum, l) => sum + l.t, 0) || 1;
      let acc = 0;
      for (const lay of pocheStack) {
        const f0 = acc / tot, f1 = (acc + lay.t) / tot;
        acc += lay.t;
        out.push({
          pts: [lerpP(l0, r0, f0), lerpP(l1, r1, f0), lerpP(l1, r1, f1), lerpP(l0, r0, f1)],
          closed: true, role: 'poche', hatch: wallHatchOn(w), mat: lay.fill || null,
        });
      }
    } else {
      out.push({ pts: [l0, l1, r1, r0], closed: true, role: 'poche', hatch: wallHatchOn(w) });
    }
  }
  /* and the patch in the middle of a junction, from whichever wall owns it */
  for (const end of [0, 1]) {
    const P = end ? w.b : w.a;
    if (nodePatchOwner(P, w.lvl || 0) !== w.id) continue;
    const patch = nodePatch(P, w.lvl || 0);
    /* poche, so it is filled, plotted and screened exactly as a wall body is —
       and flagged, because it belongs to a junction rather than to a layer of
       one wall and must not be given a material's pattern */
    if (patch) out.push({ pts: patch, closed: true, role: 'poche', node: true,
                          hatch: wallHatchOn(w) });
  }
  ...
  for (const o of winCuts) {
    ...
    out.push({
      pts: [faceAtD(Lcar, w, u, n, s), faceAtD(Lcar, w, u, n, e),
        faceAtD(Rcar, w, u, n, e), faceAtD(Rcar, w, u, n, s)],
...
GEOM.wall = {
  ...
  /* the run of the wall, which is what gets ordered and scheduled */
  len: w => wallLen(w),
};
