...
function doSaveDWG() {
  modal(`<h3>Export DWG — experimental</h3>
    ...
    cleanly through Orthograph's own reader — and <b>AutoCAD rejects it</b>, at the file header,
    with "Drawing was created by an incompatible version". It has been reported and not yet fixed.</p>
    <p>Use <b>DXF R2000</b>. That path is checked two ways and opens in AutoCAD.</p>`, () => {
    try {
      const buf = exportDWG();
      download('drawing.dwg', new Blob([buf], { type: 'application/acad' }));
      toast('Wrote drawing.dwg — AutoCAD is known to refuse it; use DXF');
