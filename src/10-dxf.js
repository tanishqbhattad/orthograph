...
class DxfWriter {
  ...
  tablesSection() {
    ...
    const tbl = (name, handle, count, body) => {
      ...
      /* DIMSTYLE is the one symbol table whose header carries a SECOND
         subclass marker, with its own count after it. Every other table stops
         at AcDbSymbolTable. Without it AutoCAD reaches the first record where
         it expected the marker and discards the whole drawing:

           Class separator for class AcDbDimStyleTable expected on line 828.
           Invalid or incomplete DXF input -- drawing discarded.

         ezdxf reads the file happily either way, which is why every check
         here passed while AutoCAD would not open it at all. */
      if (name === 'DIMSTYLE') { P(o, 100, 'AcDbDimStyleTable'); P(o, 71, count); }
      body(); P(o, 0, 'ENDTAB');
    };
    ...
    this.layerHandles = {};
    /* Every layer any exported object NAMES, whether or not the document has a
       record for it. A drawing can hold an object on a layer nobody made —
       AUDIT exists to find exactly that — and a file that references a layer
       it does not define loses it: AutoCAD drops those objects onto layer 0.
       The document is allowed to be untidy; the file it writes is not. */
    const layersOut = DOC.layers.slice();
    {
      const have = new Set(layersOut.map(l => l.name));
      const named = new Set();
      for (const e of DOC.ents.values()) if (e && e.layer != null) named.add(String(e.layer));
      for (const b of Object.values(DOC.blocks || {})) {
        const list = Array.isArray(b) ? b : (b && b.ents) || [];
        for (const e of list) if (e && e.layer != null) named.add(String(e.layer));
      }
      for (const n of named) {
        if (n === '' || have.has(n)) continue;
        layersOut.push(Object.assign(newLayer(n), { name: n }));
        have.add(n);
      }
    }
    tbl('LAYER', K.tLayer, layersOut.length, () => {
