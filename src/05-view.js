...
/* ============================================================
   ...
   A pattern is N families of parallel lines. Each family is an angle, an
   origin, the perpendicular spacing between its lines, the shift along the
   line from one to the next — which is what makes a running bond stagger —
   and an optional dash pattern. Everything below is expressed in multiples of
   the hatch's own spacing, so one definition serves every scale.

   These definitions are ours. The names are the industry ones so that a DXF
   round trip means something to whoever opens it, but the numbers were
   authored here: the .pat libraries shipped with other CAD are someone else's
   work under someone else's licence.

     a     angle in degrees, added to the hatch's own rotation
     dy    spacing between the lines of this family
     dx    shift along the line between consecutive lines
     y0    offset of the whole family across its lines
     dash  [on, off, ...] in the same units; absent means solid
   ============================================================ */
const HATCH_PATTERNS = {
  /* the two we have always had, under their real names */
  'line':     [{ a: 45, dy: 1 }],
  'cross':    [{ a: 45, dy: 1 }, { a: 135, dy: 1 }],
  'ANSI31':   [{ a: 45, dy: 1 }],
  'ANSI32':   [{ a: 45, dy: 1 }, { a: 45, dy: 1, y0: 0.3 }],
  'ANSI37':   [{ a: 45, dy: 1 }, { a: 135, dy: 1 }],

  /* masonry: courses, with head joints staggered half a brick each course */
  'AR-BRSTD': [{ a: 0, dy: 1 },
               { a: 90, dy: 2, dx: 1, dash: [1, 1] }],
  'AR-B816':  [{ a: 0, dy: 2 },
               { a: 90, dy: 4, dx: 2, dash: [2, 2] }],
  'AR-HBONE': [{ a: 45, dy: 1, dash: [3, 1] },
               { a: 135, dy: 1, dx: 1.5, dash: [3, 1] }],

  /* concrete: aggregate suggested by short strokes at three angles, which is
     what the drawn convention actually is */
  'AR-CONC':  [{ a: 50, dy: 1.6, dash: [0.7, 2.3] },
               { a: 355, dy: 2.1, dx: 0.8, dash: [0.5, 3.1] },
               { a: 100, dy: 2.6, dx: 1.3, dash: [0.4, 2.7] }],
  'AR-SAND':  [{ a: 30, dy: 0.9, dash: [0.1, 1.7] },
               { a: 120, dy: 1.1, dx: 0.5, dash: [0.1, 1.9] }],
  'PLAST':    [{ a: 20, dy: 0.7, dash: [0.15, 1.2] },
               { a: 110, dy: 0.8, dx: 0.4, dash: [0.15, 1.4] }],

  /* insulation: the batting loop, approximated as offset dashes between two
     containing lines, which is how it reads at plan scale anyway */
  'INSUL':    [{ a: 0, dy: 2 },
               { a: 0, dy: 2, y0: 1.2 },
               { a: 90, dy: 0.9, dy0: 0, dash: [0.8, 1.2] }],

  'EARTH':    [{ a: 45, dy: 1.2, dash: [2, 1.5] },
               { a: 45, dy: 1.2, y0: 0.5, dash: [1, 2.5] }],
  'GRAVEL':   [{ a: 15, dy: 1.4, dash: [0.3, 2.2] },
               { a: 75, dy: 1.6, dx: 0.7, dash: [0.3, 2.6] }],
  'STEEL':    [{ a: 45, dy: 1.6 },
               { a: 45, dy: 1.6, y0: 0.35 }],
  'TIMBER':   [{ a: 0, dy: 1 },
               { a: 0, dy: 1, y0: 0.25, dash: [4, 3] }],
  'NET':      [{ a: 0, dy: 1 }, { a: 90, dy: 1 }],
  'TRIANG':   [{ a: 0, dy: 1.7 }, { a: 60, dy: 1.7 }, { a: 120, dy: 1.7 }],
  'HONEY':    [{ a: 0, dy: 1.7, dash: [1, 2] },
               { a: 60, dy: 1.7, dash: [1, 2] },
               { a: 120, dy: 1.7, dash: [1, 2] }],
  'DOTS':     [{ a: 0, dy: 1, dash: [0.05, 1.95] },
               { a: 90, dy: 1, dash: [0.05, 1.95] }],
};
/** every pattern by name, for a dialog to offer */
function hatchPatterns() { return Object.keys(HATCH_PATTERNS); }
/** the families of a pattern; an unknown name still draws as plain lines,
    because a hatch we cannot name is still a hatch somebody put there */
function hatchPattern(name) {
  return HATCH_PATTERNS[String(name || 'line')] ||
...
/** Draw a material pattern inside one closed band — a wall layer, in the
    ...
    hatch-shaped record and handing it over, so a pattern cannot look one way
    in a hatch and another in a wall. */
function hatchBands(pts, pat, col, e) {
  if (!pts || pts.length < 3) return;
  const sp = (typeof wallPatSpacing === 'function') ? wallPatSpacing(e) : 120;
  drawHatch({ t: 'hatch', loops: [pts], pattern: pat, sp, hatchAng: 0 }, col, null);
}
/** How big a wall's material pattern is drawn. Tied to the wall's thickness
    rather than fixed, so a 300 cavity and a 100 partition both read. */
function wallPatSpacing(e) {
  const t = (e && e.t === 'wall' && typeof wallT === 'function') ? wallT(e) : 200;
  return Math.max(t / 3, 40);
}
  ...
  if (!HALO) {
    ...
    if (e.solid) {
      ...
      if (isFinite(diag) && diag > 0 && diag / sp < 6000) {
        ...
        ctx.lineWidth = HAIR;
        /* One rotate-and-stroke pass per family. Everything a family holds is
           a multiple of the hatch's own spacing, so a definition drawn at 1:50
           and at 1:200 is the same pattern at two sizes. */
        for (const f of hatchPattern(e.pattern)) {
          const gap = Math.max((f.dy || 1) * sp, 2);
          if (diag / gap > 4000) continue;         /* denser than it is worth drawing */
          ctx.save();
          ctx.translate(cx, cy);
          ctx.rotate(-(a + rad(f.a || 0) + V.rot));
          ctx.setLineDash(f.dash ? f.dash.map(d => Math.max(d * sp, 0.5)) : DASH_SOLID);
          const shift = (f.dx || 0) * sp;
          const off = (f.y0 || 0) * sp;
          let i = 0;
          ctx.beginPath();
          for (let y = -diag / 2 + off; y <= diag / 2; y += gap, i++) {
            /* the shift along the line is what staggers a running bond, and
               it is expressed by phasing the dash rather than moving the ends */
            if (shift) { ctx.stroke(); ctx.beginPath(); ctx.lineDashOffset = -i * shift; }
